...
const translationsData = {
  en: {
    title: 'The ID/Password <span class="colored-word">Tactic</span>',
    ...
    howText: '<span class="highlight">User Input:</span> The user enters their ID and password.<span className="highlight">Verification:</span> The system checks if the credentials match the stored data.<span className="highlight">Access Decision:</span> If correct → User is granted access. If not, Access is denied.<span className="highlight">(Optional) Additional Security Measures:</span> Systems may lock the account after multiple failed attempts or require CAPTCHA verification.',
  ...
  fr: {
    title: 'La tactique ID/Password <span class="colored-word">Tactic</span>',
    ...
    howText: '<span class="highlight">Saisie des identifiants :</span> L\'utilisateur entre son identifiant et son mot de passe.<span className="highlight">Vérification :</span> Le système compare les données saisies avec celles enregistrées.<span className="highlight">Décision d\'accès :</span> Si correct → L\'accès est accordé. Sinon → L\'accès est refusé.<span className="highlight">(Optionnel) Mesures de sécurité supplémentaires :</span> Les systèmes peuvent bloquer le compte après plusieurs échecs ou exiger une vérification CAPTCHA.',
